...
void addColorToVector(glm::vec3 color, std::vector<float>& vector){
...
glm::vec2 Mountain::sampleRandomVector(int row, int col)
{
...
glm::vec3 Mountain::getPosition(int row, int col) {
...
float Mountain::getHeight(float x, float y) {
...
glm::vec3 Mountain::getNormal(int row, int col) {
...
//    result = glm::vec3(result.x, result.z, result.y);
    return result;
}
...
//    glm::vec3 upRight(0, 1, 0);
    ...
    if(std::abs(theta) < std::numbers::pi / 8){
//        position[2] > 0.05f &&
        color = white;
    }
...
float Mountain::computePerlin(float x, float y) {
